...
namespace CWJ.Singleton.Core
{
    ...
    public abstract class SingletonCoreAbstract<T> : SingletonCore where T : MonoBehaviour
    {
        ...
        /// </summary>
        protected virtual void OnBeforeInstanceAssigned() { }
        ...
        protected static T _Instance
        {
            ...
            set
            {
                if (__INSTANCE != null)
                {
                    (__INSTANCE as SingletonCoreAbstract<T>)._isInstance = false;
                    SingletonHelper.RemoveSingletonInstanceElem(__INSTANCE);
                    __INSTANCE = null;
                }

                var valueOfCore = value != null ? (value as SingletonCoreAbstract<T>) : null;
                if (valueOfCore != null)
                {
                    valueOfCore.OnBeforeInstanceAssigned();
                }
        ...
        protected void SetDontDestroyOnLoad()
        {
            ...
            {
                typeof(SingletonCore).PrintLogWithClassName($"{TargetTypeName}때문에 '{transform.root.name}'(오브젝트이름)에 DontDestroyOnLoad를 실행했습니다", LogType.Log, false, obj: gameObject, isPreventStackTrace: true);
            }
        }
