    ...
    public class CoroutineTracked : IDisposable
    {
...
#if UNITY_EDITOR && CWJ_EXISTS_EDITORCOROUTINE
        private EditorCoroutine editor_wrapperCoroutine;
        private EditorCoroutine editor_doCoroutine;
#endif
        ...
        IEnumerator Routine(MonoBehaviour monoBehaviour, IEnumerator enumerator)
        {
            ...
            if (!UnityEditor.EditorApplication.isPlaying)
                yield return (editor_doCoroutine = EditorCoroutineUtility.StartCoroutine(enumerator, monoBehaviour));
            else
#endif
...
#if UNITY_EDITOR && CWJ_EXISTS_EDITORCOROUTINE
                if (!UnityEditor.EditorApplication.isPlaying)
                    editor_wrapperCoroutine = EditorCoroutineUtility.StartCoroutine(Routine(behaviour, enumerator), behaviour);
                else
#endif
...
#if UNITY_EDITOR && CWJ_EXISTS_EDITORCOROUTINE
                    if (!UnityEditor.EditorApplication.isPlaying)
                        EditorCoroutineUtility.StartCoroutine(WaitForDoRoutineExists(behaviour), behaviour);
                    else
#endif
        ...
        private void _AllStopCor()
        {
            ...
            wrapperCoroutine = null;
#if UNITY_EDITOR && CWJ_EXISTS_EDITORCOROUTINE
            if (editor_doCoroutine != null) EditorCoroutineUtility.StopCoroutine(editor_doCoroutine);
            editor_doCoroutine = null;
            if (editor_wrapperCoroutine != null) EditorCoroutineUtility.StopCoroutine(editor_wrapperCoroutine);
            editor_wrapperCoroutine = null;
#endif
            _isRunning_frameSafely = _isRunning_insideCor = false;
            isStopping = false;
